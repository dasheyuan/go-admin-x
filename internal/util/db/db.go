...
import (
	...
	"go-admin-x/internal/util/errors"
...
func execContext(ctx context.Context, name string, db unionDB, query Query, args []interface{}) (sql.Result, error) {
	...
	log.Get().WithField("cost", cost).Debugf("[DB] name:%s sql:%s args:%v", name, query.sql, args)
...
func (tx *Tx) log(ctx context.Context, msg string) {
	...
	if msg == "commit" {
		log.Get().Debugf("commit, total cost:%s", duration)
	} else {
		log.Get().Warnf("rollback, total cost:%s", duration)
	}

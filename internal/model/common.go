...
func Create(value interface{}) error {
	return orm.DB.Create(value).Error
}

// Save
func Save(value interface{}) error {
	return orm.DB.Save(value).Error
}

// Updates
func Updates(where interface{}, value interface{}) error {
	return orm.DB.Model(where).Updates(value).Error
}

// Delete
func DeleteByModel(model interface{}) (count int64, err error) {
	db := orm.DB.Delete(model)
	err = db.Error
	if err != nil {
		return
	}
...
func DeleteByWhere(model, where interface{}) (count int64, err error) {
	db := orm.DB.Where(where).Delete(model)
	err = db.Error
	if err != nil {
		return
	}
...
func DeleteByID(model interface{}, id uint64) (count int64, err error) {
	db := orm.DB.Where("id=?", id).Delete(model)
	err = db.Error
	if err != nil {
		return
	}
...
func DeleteByIDS(model interface{}, ids []uint64) (count int64, err error) {
	db := orm.DB.Where("id in (?)", ids).Delete(model)
	err = db.Error
	if err != nil {
		return
	}
...
func First(where interface{}, out interface{}) (notFound bool, err error) {
	err = orm.DB.Where(where).First(out).Error
	if err != nil {
		notFound = gorm.IsRecordNotFoundError(err)
	}
	return
...
func Scan(model, where interface{}, out interface{}) (notFound bool, err error) {
	err = orm.DB.Model(model).Where(where).Scan(out).Error
	if err != nil {
		notFound = gorm.IsRecordNotFoundError(err)
	}
	return
...
func ScanList(model, where interface{}, out interface{}, orders ...string) error {
	db := orm.DB.Model(model).Where(where)
	if len(orders) > 0 {
		for _, order := range orders {
			db = db.Order(order)
		}
...
func PluckList(model, where interface{}, out interface{}, fieldName string) error {
	return orm.DB.Model(model).Where(where).Pluck(fieldName, out).Error
}

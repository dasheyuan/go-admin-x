...
// 删除用户及关联数据
func (Admin) Delete(adminids []uint64) error {
	tx := orm.DB.Begin()
	defer func() {
		if r := recover(); r != nil {
			tx.Rollback()
	...
	if err := tx.Where("id in (?)", adminids).Delete(&Admin{}).Error; err != nil {
		tx.Rollback()
		return err
	}
	if err := tx.Where("admins_id in (?)", adminids).Delete(&AdminRole{}).Error; err != nil {
		tx.Rollback()
		return err
	}

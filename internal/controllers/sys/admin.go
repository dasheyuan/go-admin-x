...
func (Admin) List(c *gin.Context) {
	...
	var total uint64
	list := []sys.Admin{}
	err := models.GetPage(&sys.Admin{}, &sys.Admin{}, &list, page, limit, &total, whereOrder...)
	if err != nil {
...
// 详情
func (Admin) Detail(c *gin.Context) {
...
// 获取用户下的角色ID列表
func (Admin) AdminsRoleIDList(c *gin.Context) {
	adminsid := common.GetQueryToUint64(c, "adminsid")
	roleList := []uint64{}
	where := sys.AdminRole{AdminID: adminsid}
	err := models.PluckList(&sys.AdminRole{}, &where, &roleList, "role_id")
	if err != nil {
		common.ResErrSrv(c, err)
		return
	}
...
// 分配用户角色权限
func (Admin) SetRole(c *gin.Context) {
	adminsid := common.GetQueryToUint64(c, "adminsid")
	var roleids []uint64
	err := c.Bind(&roleids)
	if err != nil {
